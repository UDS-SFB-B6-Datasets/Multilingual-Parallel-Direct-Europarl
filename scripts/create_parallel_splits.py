...
def main(infile, outdir, src_langs, parallel_langs, native, direct,
    ...
    L = df.src.value_counts().min()
    a, b, c = get_proportions(L, [tsplit, dsplit])
    print(L, a, b, c)

    train_splits, dev_splits, test_splits = {}, {}, {}
    for k in src_langs:
        train_splits[k], dev_splits[k], test_splits[k] = split(df[df.src == k].sample(L), a, b, c)

    train_df = pandas.concat(train_splits.values())
    dev_df = pandas.concat(dev_splits.values())
    test_df = pandas.concat(test_splits.values())

    print(test_df.src.value_counts())

    outdir = os.path.dirname(outdir)
    if not os.path.exists(outdir):
        os.makedirs(outdir)
    train_df.to_csv(os.path.join(outdir, "parallel_train.tsv"), sep="\t", index=False)
    dev_df.to_csv(os.path.join(outdir, "parallel_dev.tsv"), sep="\t", index=False)
    test_df.to_csv(os.path.join(outdir, "parallel_test.tsv"), sep="\t", index=False)
